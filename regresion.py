...
def main():
    ...
    if dup_dataset is not None:
        ...
        data_mae = {'Model' : a_mae, 'Accuracy' : b_mae}
        data_mae = pd.DataFrame(data_mae)

        data_mse = {'Model': a_mse, 'Accuracy': b_mse}
        data_mse = pd.DataFrame(data_mse)

        data_r2 = {'Model': a_r2, 'Accuracy': b_r2}
        data_r2 = pd.DataFrame(data_r2)

        # Graphs
        import plotly.express as px
        fig = px.bar(data_mae, x='Model', y='Accuracy', color='Accuracy')
        st.plotly_chart(fig)

        import plotly.express as px
        fig = px.bar(data_mse, x='Model', y='Accuracy', color='Accuracy')
        st.plotly_chart(fig)

        import plotly.express as px
        fig = px.bar(data_r2, x='Model', y='Accuracy', color='Accuracy')
        st.plotly_chart(fig)